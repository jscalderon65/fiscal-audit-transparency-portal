  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <div className="mt-8 flex items-center gap-2 sm:gap-3">
        ...
        {stepLine(step >= 4)}
        {stepCircle(4, "Dictámenes", step === 4, step > 4)}
      </div>
      ...
      {step === 4 && (
        <div className="mt-8 space-y-6">
          <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm">
            ...
            <Text className="text-slate-500 text-sm mb-4">Agrega los dictámenes mensuales que los residentes podrán consultar y descargar.</Text>
            ...
            {pendingReports.map((report, index) => (
              <div key={index} className="mb-4 p-4 rounded-xl bg-slate-50 border border-slate-200 relative">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pr-8">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">Título</label>
                    <input type="text" value={report.title} onChange={(e) => updateReport(index, "title", e.target.value)} placeholder="Ej: Dictamen Revisoría Fiscal - Marzo 2026" className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm outline-none focus:border-primary bg-white text-slate-900" />
                  </div>
                  <div className="sm:col-span-2">
            ...
            <Button variant="outline" leftIcon={Plus} onClick={addReport}>Agregar dictamen</Button>
          </div>
          <div className="flex items-center justify-between">
